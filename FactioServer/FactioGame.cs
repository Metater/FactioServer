...
public class FactioGame : ITickable
{
    ...
    private readonly Dictionary<FactioPlayer, string> playerAResponses = new(); // player, response
    private readonly Dictionary<FactioPlayer, string> playerBResponses = new(); // player, response

    private readonly List<Scenario> scenarios = new();

    private readonly Dictionary<FactioPlayer, bool> votes = new(); // player, is vote player b
    private readonly Dictionary<FactioPlayer, int> scores = new(); // player, score

    private int roundIndex = 0;

    private List<FactioPlayer> unchosenPlayers = new();
    private readonly List<(FactioPlayer, FactioPlayer)> chosenPairs = new();
    ...
    private void TickPhaseVoting()
    {
        if (ShouldPhaseInit())
        {
            ...
            VotingStartCPacket votingStart = new()
    ...
    private void TickPhaseResults()
    {
        if (ShouldPhaseInit())
        {
            ...
            ResultsStartCPacket resultsStart = new()
    ...
    private void TickPhaseRoundResults()
    {
        if (ShouldPhaseInit())
        {
            ...
            // TODO Sortin dont work
            List<KeyValuePair<FactioPlayer, int>> unsortedScores = new(scores);
            List<KeyValuePair<FactioPlayer, int>> sortedScores = new();
            for (int i = 0; i < scores.Count || i < 10; i++)
            {
                KeyValuePair<FactioPlayer, int> greatestScore = new(null, int.MinValue);
            ...
            RoundResultsStartCPacket roundResultsStart = new()
    ...
    private void StartRound()
    {
        ...
        while (unchosenPlayers.Count > 1)
        {
            ...
            RoundStartCPacket participantsRoundStart = new()
            {
    ...
    {
        CloseLobbyCPacket closeLobby = new()
        { Reason = (byte)reason };
        players.ForEach((p) =>
